...
public class RewardList implements Gui{
	...
	private GenericLabel label = (GenericLabel) new GenericLabel().setText("Reward List").setHeight(15).shiftXPos(- 15).setAnchor(WidgetAnchor.TOP_CENTER);
	...
	public RewardList(RpgQuests plugin, SpoutPlayer splayer, Quest quest){
		this.plugin = plugin;
		this.splayer = splayer;
		this.quest = quest;
		page = 0;
		maxPage = (int) (Math.ceil(quest.getRewards().length/9.0) -1);
	...
	private void createPopup(boolean attach, boolean remove) {
		...
		if(i2>quest.getRewards().length){
			i2 = quest.getRewards().length;
		}
		
		popup.removeWidgets(plugin);
		
		//popup.attachWidget(plugin, new GenericLabel("ID").setWidth(50).setHeight(20).shiftXPos(-152).setY(Y).setAnchor(WidgetAnchor.TOP_CENTER));
		...
		popup.attachWidget(plugin, new GenericLabel("Delete").setWidth(50).setHeight(20).shiftXPos(+157).setY(Y + 5).setAnchor(WidgetAnchor.TOP_CENTER));
		
		for(int row = i1; row < i2; row++){
			int pos = (page > 0 ? row-(page*9) : row) + 1;
			
			//popup.attachWidget(plugin, new GenericLabel(String.valueOf(quest.getRewards()[row].getId())).setWidth(100).setHeight(20).shiftXPos(-150).setY(Y + (pos * 20)).setAnchor(WidgetAnchor.TOP_CENTER));
			
			popup.attachWidget(plugin, new GenericLabel(String.valueOf(quest.getRewards()[row].getType())).setWidth(200).setHeight(20).shiftXPos(-20).setY(Y + 5 + (pos * 20)).setAnchor(WidgetAnchor.TOP_CENTER));
