	...
	public static void command(String args[], Player player, SpoutPlayer splayer, CommandSender sender){
		if(player == null){
			...
			if(args.length == 1){//npc create
				...
				if(plugin.hasPermission(player, "rpgessentials.npc.create") || plugin.hasPermission(player, "npc.admin")){
				...
				if(plugin.hasPermission(player, "rpgessentials.npc.create") || plugin.hasPermission(player, "npc.admin")){
					if(args[2].equalsIgnoreCase("default") || args[2].equalsIgnoreCase("banker") || args[2].equalsIgnoreCase("quester")){
						...
						plugin.m.spawnHumanNPC(args[1], location);
						player.sendMessage(ChatColor.GREEN + "NPC: " + ChatColor.YELLOW + args[1] + ChatColor.GREEN + " with type:" + args[2].toLowerCase() +" has been created!");
					}else{
