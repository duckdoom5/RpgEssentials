...
public class NpcArmour extends RpgEssentialsCommandExecutor{
	public NpcArmour(RpgEssentials instance) {
		super(instance);
	}
	...
	public static void command(String args[], Player player, SpoutPlayer splayer, CommandSender sender){
		if(player == null){
			...
			if(args.length == 1){//npc armour
				if(plugin.hasPermission(player, "rpgessentials.npc.armour") || plugin.hasPermission(player, "npc.admin")){
					String id = npc.getSelected(player);
					if(id != null){
						HumanNPC hnpc = (HumanNPC) plugin.m.getNPC(id);
						...
						player.sendMessage(ChatColor.YELLOW + id + "'s " + ChatColor.GREEN + "armour has been cleared");
