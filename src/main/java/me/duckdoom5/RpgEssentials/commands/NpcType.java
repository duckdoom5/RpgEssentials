	...
	public static void command(String args[], Player player, SpoutPlayer splayer, CommandSender sender){
		if(player == null){
			...
			if(args.length == 1){//npc type
				...
				if(plugin.hasPermission(player, "rpgessentials.npc.type") || plugin.hasPermission(player, "npc.admin")){
					if(args[1].equalsIgnoreCase("default") || args[1].equalsIgnoreCase("banker") || args[1].equalsIgnoreCase("quester")){
						...
						if(id != null){
							if(!Configuration.npc.getString("Npc." + id + ".type").equals(args[1].toLowerCase())){
								...
								}else if(Configuration.npc.contains("Npc." + id + ".quest")){
									Configuration.npc.set("Npc." + id + ".quest", null);
								}
