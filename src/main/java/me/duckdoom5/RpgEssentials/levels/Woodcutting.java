...
public class Woodcutting {
	...
	public static void canuse(int currentlevel, Block block, Player player, RpgEssentials plugin, ItemStack inhand, BlockBreakEvent event) {
		...
		}else if(((currentlevel >= Configuration.level.getInt("UnlockLevel.Stone Axe") || Bukkit.getPluginManager().isPluginEnabled("WorldGaurd") == true) && currentlevel < Configuration.level.getInt("UnlockLevel.Iron Axe")) && (inhand.getType() == Material.WOOD_AXE || inhand.getType() == Material.STONE_AXE)){
			Woodcutting.blockcheck(block, player, plugin);
