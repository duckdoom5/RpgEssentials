...
public class StoreMethods {
    ...
    public static void buyClick(SpoutPlayer splayer, Button button) {
        ...
        final StoreGui gui = (StoreGui) GuiManager.gui.get(splayer);
    ...
    private static void buyCustom(org.getspout.spoutapi.material.Material customMaterial, int amount, SpoutPlayer splayer, String type) {
        ...
        final double price = Configuration.store.getInt("Store.custom." + type + "." + customMaterial.getName() + ".Buy Price") * amount;
        if (money < price) {
    ...
    private static void buyData(ItemStack dataMaterial, int amount, SpoutPlayer splayer, String type) {
        final double price = Configuration.store.getDouble("Store." + type + "." + dataMaterial.getType().toString().toLowerCase().replace("_", " ") + ".Buy Price") * amount;
        double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
        if (money < price) {
    ...
    private static void buyMaterial(Material material, int amount, SpoutPlayer splayer, String type) {
        final double price = Configuration.store.getDouble("Store." + type + "." + material.toString().toLowerCase().replace("_", " ") + ".Buy Price") * amount;
        double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
        if (money < price) {
            splayer.sendNotification("Error", "Not Enough Money!", new ItemStack(Material.DIAMOND_SWORD), 2000);
        } else {
    ...
    private static void sellCustom(org.getspout.spoutapi.material.Material customMaterial, int amount, SpoutPlayer splayer, String type) {
        double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
        final double price = Configuration.store.getDouble("Store.custom." + type + "." + customMaterial.getName() + ".Sell Price") * amount;
    ...
    private static void sellData(ItemStack dataMaterial, int amount, SpoutPlayer splayer, String type) {
        final double price = Configuration.store.getDouble("Store." + type + "." + dataMaterial.getType().toString().toLowerCase().replace("_", " ") + ".Sell Price") * amount;
        double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
        ...
        if (!sell) {
            ...
            if (!gotAmount) {
                ...
                splayer.sendNotification(amount + "x " + Methods.getDataName(dataMaterial.getType(), dataMaterial.getDurability()), "Sold for: " + price + " " + me.duckdoom5.RpgEssentials.config.Configuration.config.getString("currency.value"), dataMaterial, 1000);
                // money plus price
                money = money + price;
                RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).setMoney(money);
    ...
    private static void sellMaterial(Material material, int amount, SpoutPlayer splayer, String type) {
        final double price = Configuration.store.getDouble("Store." + type + "." + material.toString().toLowerCase().replace("_", " ") + ".Sell Price") * amount;
        double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
