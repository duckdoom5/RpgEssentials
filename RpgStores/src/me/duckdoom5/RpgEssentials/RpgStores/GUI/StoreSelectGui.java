...
public class StoreSelectGui extends Gui {
    ...
    public StoreSelectGui(RpgStores plugin, SpoutPlayer splayer, int x, int y, int z) {
        ...
        if (!Configuration.storedata.getString(x + "," + y + "," + z).equalsIgnoreCase("all")) {
            StoreHashmaps.registerCustomStore(plugin, splayer,
                    Configuration.storedata.getString(x + "," + y + "," + z));
            return;
        }
        this.plugin = plugin;
    ...
    public StoreSelectGui(RpgStores plugin, SpoutPlayer splayer, String name) {
        ...
        if (!Configuration.storedata.getString(name).equalsIgnoreCase("all")) {
            StoreHashmaps.registerCustomStore(plugin, splayer,
                    Configuration.storedata.getString(name));
            return;
        }
        this.plugin = plugin;
        this.splayer = splayer;
    ...
    protected void createPopup(boolean attach, boolean remove) {
        ...
        for (int row = 0; row < names.length; row++) {
            ...
            popup.attachWidget(
                    plugin,
                    new GenericButton(names[row]).setWidth(200).setHeight(20).shiftXPos(right ? -205 : +5).setY((int) (Y + (pos * 12.5))).setAnchor(WidgetAnchor.TOP_CENTER));
        }
        final double money = RpgeManager.getInstance().getRpgPlayerManager().getRpgPlayer(splayer).getMoney();
        final GenericLabel moneylb = (GenericLabel) new GenericLabel().setText(Double.toString(money)).setHeight(15).setWidth(50).shiftXPos(-50).setAnchor(WidgetAnchor.TOP_RIGHT);
        final GenericLabel storelb = (GenericLabel) new GenericLabel().setText("Store").setHeight(15).setWidth(30).shiftXPos(-15).setAnchor(WidgetAnchor.TOP_CENTER);
