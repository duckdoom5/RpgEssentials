...
public class TextSelectMenu {
	...
	public static void open(Plugin plugin, SpoutPlayer splayer, String title, String[] text, String[] buttons) {
		...
		GenericTexture BT = (GenericTexture) new GenericTexture().setUrl("http://dl.dropbox.com/u/62672791/textures/bordertop.png").setMinWidth(160).setMaxWidth(160).setMinHeight(8).setMaxHeight(8).setPriority(RenderPriority.High).shiftYPos(-(20 * buttons.length + 15 * text.length + 31)).shiftXPos(- 160).setAnchor(WidgetAnchor.BOTTOM_RIGHT);
		GenericTexture BG = (GenericTexture) new GenericTexture().setUrl("http://dl.dropbox.com/u/62672791/textures/borderbg.png").setMinWidth(160).setMaxWidth(160).setMinHeight(20 * buttons.length + 15 * text.length + 15).setMaxHeight(20 * buttons.length + 15 * text.length + 15).setPriority(RenderPriority.High).shiftYPos(-(20 * buttons.length + 15 * text.length + 23)).shiftXPos(- 160).setAnchor(WidgetAnchor.BOTTOM_RIGHT);
		GenericTexture BB = (GenericTexture) new GenericTexture().setUrl("http://dl.dropbox.com/u/62672791/textures/borderbotom.png").setMinWidth(160).setMaxWidth(160).setMinHeight(8).setMaxHeight(8).setPriority(RenderPriority.High).shiftYPos(- 8).shiftXPos(- 160).setAnchor(WidgetAnchor.BOTTOM_RIGHT);
