...
import me.duckdoom5.RpgEssentials.RpgPets.RpgPets;
...
public static MyConfiguration config;
	...
	public static void start(){
		RpgPets.log.info(h+"Static Configuration loading...");
	...
	private static void exclaim(String filename){
		RpgPets.log.info(h+"Saved file "+ filename + "!");
	}

	private static void complain(String filename){
		RpgPets.log.severe(h+"On file "+ filename + ":");
		RpgPets.log.severe(h+"Invalid configuration! Did you use tabs or restrict permissions?");
	}

	private static void complainFileCreation(String filename){
		RpgPets.log.severe(h+"On file "+ filename + ":");
		RpgPets.log.severe(h+"Could NOT create default files! Did you restrict permissions?");
	}

// return true if defaults need to be created
	private static boolean load(MyConfiguration y, String name){
		try {
			y.load("plugins/RpgPets/"+name);
	...
	private static void save(MyConfiguration y){
		try {
			y.save();
			try {
				y.load("plugins/RpgPets/" + y.getFilename());
			} catch (Exception e) {
			}
