...
public class SpawnerWandGui extends Gui {
    ...
    private final GenericLabel type = (GenericLabel) new GenericLabel().setHeight(15).setWidth(30).shiftXPos(-50).setAnchor(WidgetAnchor.TOP_CENTER);
    ...
    public SpawnerWandGui(RpgEssentials plugin, SpoutPlayer splayer,
    ...
    protected void createPopup(boolean attach, boolean remove) {
        ...
        for (int row = 0; row < names.length; row++) {
            ...
            popup.attachWidget(
                    plugin,
                    new GenericButton(names[row]).setWidth(100).setHeight(20).shiftXPos(right ? -160 : center ? -50 : +60).setY((int) (Y + (pos * 25 / 3.0))).setAnchor(WidgetAnchor.TOP_CENTER));
        }
