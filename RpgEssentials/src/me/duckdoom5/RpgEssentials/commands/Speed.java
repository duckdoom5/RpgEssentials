...
public class Speed extends RpgEssentialsCommandExecutor{
	...
	public static void command(String args[], Player player, SpoutPlayer splayer, CommandSender sender){
		if(args.length == 1){
			if(player != null){
				if(plugin.hasPermission(player, "rpgessentials.rpg.speed")){
					splayer.resetMovement();
					ConfigAdd.addSpeed(player, 1.0);
			...
			if(player != null){
				if(plugin.hasPermission(player, "rpgessentials.rpg.speed")){
					...
					ConfigAdd.addSpeed(player, multi);
			...
			if(plugin.hasPermission(player, "rpgessentials.rpg.speed.other")){
				...
				if(P == null){
					...
					sender.sendMessage(ChatColor.GREEN + "Speed has been set to " + ChatColor.YELLOW + args[1] + ChatColor.GREEN + " for player: " + ChatColor.AQUA + P.getName() + ChatColor.GREEN + " !");
					P.sendMessage(ChatColor.GREEN + "Your speed has been set to " + ChatColor.YELLOW + args[1] + ChatColor.GREEN + " by player: " + ChatColor.AQUA + (player!=null?player.getName():"CONSOLE") + ChatColor.GREEN + " !");
				}
			} else {
				permissions(player);
			}
			...
			sender.sendMessage(ChatColor.AQUA + "Usage: /rpg speed "+ ChatColor.GREEN + "[player] " + ChatColor.RED +  "{speed}");
		}
	}
}
