...
public class Skin extends RpgEssentialsCommandExecutor{
	...
	public static void command(String args[], Player player, SpoutPlayer splayer, CommandSender sender){
		if(args.length == 1){//rpg skin
			if(player != null){
				if(plugin.hasPermission(player, "rpgessentials.rpg.skin")){
					splayer.resetSkin();
					ConfigAdd.addSkin(player, "Default");
			...
			if(player != null){
				if(plugin.hasPermission(player, "rpgessentials.rpg.skin")){
					if(args[1].contains(".png")){
						...
						ConfigAdd.addSkin(player, args[1]);
			...
			if(plugin.hasPermission(player, "rpgessentials.rpg.skin.other")){
			...
			sender.sendMessage(ChatColor.AQUA + "Usage: /rpg skin "+ ChatColor.GREEN + "[player] " + ChatColor.RED +  "{url}");
		}
	}
}
