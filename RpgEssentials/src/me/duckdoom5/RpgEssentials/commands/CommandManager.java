...
public class CommandManager implements CommandExecutor {
        ...
        if (cmd.getName().equalsIgnoreCase("rpg")) {
            ...
            if (commands.containsKey(args[0])) {
                ...
                if (player != null) {
                    ...
                    final String url = Configuration.texture.getString(player.getWorld().getName() + "." + rpgplayer.getTexturepack(player.getWorld()) + ".url");
                    player.sendMessage(url);
                }
                return true;
            } else if (args[0].equals("reload")) {
                Reload.command(args, player, splayer);
                return true;
            } else if (args[0].equals("texturepack")) {
                Texturepack.command(args, player, splayer);
                return true;
            } else if (args[0].equals("mail")) {// rpg mail
                Mail.command(args, player, splayer);
                return true;
            } else if (args[0].equals("gen")) {
                Generator.command(args, player, splayer);
                return true;
            }
    ...
    public static void notEnoughArgs(Command command, CommandSender sender) {
        ...
        for (final String s : command.getRequiredArguments()) {
            msg += ChatColor.RED + "{" + s + "} ";
        }
        for (final String s : command.getOptionalArguments()) {
