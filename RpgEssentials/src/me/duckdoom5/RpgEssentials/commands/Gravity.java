		...
		if(args.length == 1){
			...
			if(plugin.hasPermission(player, "rpgessentials.rpg.gravity.other")){
				...
				if(P == null){
					...
					P.sendMessage(ChatColor.GREEN + "Your gravity has been set to " + ChatColor.YELLOW + args[1] + ChatColor.GREEN + " by player: " + ChatColor.AQUA + (player!=null?player.getName():"CONSOLE") + ChatColor.GREEN + "!");
				}
			} else {
				permissions(player);
			}
