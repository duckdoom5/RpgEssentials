...
public class Generator extends ChunkGenerator {
    ...
    public short[][] generateExtBlockSections(World world, Random random, int chunkX, int chunkZ, BiomeGrid biomes) {
        final short[][] result = new short[world.getMaxHeight() / 16 + 1][];
        int x, y, z;

        final int height = Configuration.generator.getInt("Generator.world.height");
        final double spread = Configuration.generator.getDouble("Generator.world.spread");

        final Random rand = new Random(world.getSeed());

        final SimplexOctaveGenerator octave = new SimplexOctaveGenerator(rand, height);

        octave.setScale(1 / spread);

        final int bedrock = Configuration.generator.getInt("Generator.bedrock.frequency");

        final int sealevel = Configuration.generator.getInt("Generator.sea.level");
        final boolean sea = Configuration.generator.getBoolean("Generator.sea.generate");
        final boolean desert = Configuration.generator.getBoolean("Generator.desert.generate");
        final boolean beach = Configuration.generator.getBoolean("Generator.beach.generate");
        final boolean mushroom = Configuration.generator.getBoolean("Generator.mushroom island.generate");

        if (!sea) {
            if (sealogged == false) {
                RpgEssentials.getLog().info("Sea generation disabled.");
                sealogged = true;
            }
        }
        if (!desert) {
            if (desertlogged == false) {
                RpgEssentials.getLog().info("Desert generation disabled.");
                desertlogged = true;
            }
        }
        if (!beach) {
            if (beachlogged == false) {
                RpgEssentials.getLog().info("Beach generation disabled.");
                beachlogged = true;
            }
        }
        if (!mushroom) {
            if (mushroomlogged == false) {
                RpgEssentials.getLog().info("Mushroom island generation disabled.");
                mushroomlogged = true;
            }
        }
        ...
        for (x = 0; x < 16; ++x) {
            for (z = 0; z < 16; ++z) {
                ...
                final Biome biome = biomes.getBiome(x, z);
                ...
                if (y > sealevel && (biome.equals(Biome.TAIGA) || biome.equals(Biome.TAIGA_HILLS))) {
                    final int id = getBlockIdAt(result, x, y, z);
