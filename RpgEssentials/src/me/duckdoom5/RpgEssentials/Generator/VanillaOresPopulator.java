...
import me.duckdoom5.RpgEssentials.MaterialManager;
...
public class VanillaOresPopulator extends BlockPopulator {
    ...
    public void populate(World world, Random random, Chunk chunk) {
        ...
        for (final OriginalOre block : MaterialManager.originalores) {
            freq = block.getfreq();
            minY = block.getminY();
            maxY = block.getmaxY();
            size = block.getSize();
            ...
            for (int j = 0; j < freq; j++) {
                ...
                final Chunk cur_chunk = Bukkit.getServer().getWorld(world.getName()).getChunkAt(cur_block);
